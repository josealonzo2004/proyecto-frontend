...
// ICONOS ACTUALIZADOS (Hi2 para Outline, Hi para Solid)
import { 
...
} from 'react-icons/hi'; // Usamos 'hi' para compatibilidad general, si tienes 'hi2' instalado usa ese para los Outline.
...
export const AdminDashboardPage = () => {
  ...
  const [returnFilter, setReturnFilter] = useState('pending'); // 'pending' | 'history'
  ...
  const fetchReturns = async () => {
    ...
    } catch (error) { console.error("Error devoluciones", error); }
  };
  ...
  const getStockStatus = (stock) => {
      if (stock === undefined) return { label: 'DISPONIBLE', color: 'bg-green-100 text-green-800', border: 'border-green-200' };
  ...
  // --- LÓGICA FACTURA/DEVOLUCIONES ---
  const handleOpenInvoice = (order) => {
      if (!order?.factura) return alert("Pedido sin factura generada.");
  ...
  const handleOpenInvoiceFromReturn = (devolucion) => {
      ...
      else alert("Factura no encontrada localmente.");
  };
    ...
    <div className='max-w-7xl mx-auto px-4 py-8 min-h-screen bg-gray-50'>
      ...
      {activeSection === 'dashboard' && (
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in'>
            {[
                { label: 'Ingresos Totales', value: `$${orders.reduce((acc, o) => acc + Number(o.contenidoTotal), 0).toLocaleString()}`, icon: HiOutlineChartBar, color: 'text-green-600', bg: 'bg-green-50' },
                { label: 'Pedidos Activos', value: orders.filter(o => o.estadoId === 1 || o.estadoId === 2).length, icon: HiOutlineShoppingBag, color: 'text-blue-600', bg: 'bg-blue-50' },
                { label: 'Solicitudes Devolución', value: returnsList.filter(r => r.estadoId === 1).length, icon: HiFilter, color: 'text-red-600', bg: 'bg-red-50' },
                { label: 'Usuarios Registrados', value: users.length, icon: HiOutlineUsers, color: 'text-purple-600', bg: 'bg-purple-50' },
            ].map((stat, index) => (
        ...
        <div className="animate-fade-in">
          <div className='flex flex-col sm:flex-row justify-between items-center mb-6 gap-4'>
            <div className="relative w-full sm:w-96">
                ...
                <input type="text" placeholder="Buscar producto..." className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none"
                ...
                return (
                  <div key={p.productoId} className={`bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all group ${!isActive ? 'opacity-80' : ''}`}>
                    <div className="h-48 bg-gray-100 relative overflow-hidden">
                        {p.imagen ? <img src={p.imagen} className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 ${!isActive ? 'grayscale' : ''}`} alt=""/> : <div className="flex h-full items-center justify-center text-gray-400">Sin imagen</div>}
                        <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                            <span className={`px-2 py-1 text-[10px] font-bold rounded-md border shadow-sm uppercase ${status.color} ${status.border} bg-opacity-90`}>{status.label}</span>
                            {!isActive && <span className="px-2 py-1 text-[10px] font-bold rounded-md bg-gray-800 text-white shadow-sm uppercase">BORRADOR</span>}
                        </div>
                    </div>
                    <div className="p-4">
                        <div className="flex justify-between items-start mb-2 gap-2">
                            <h3 className='font-bold text-gray-800 truncate flex-1' title={p.nombre}>{p.nombre}</h3>
                            <label className="relative inline-flex items-center cursor-pointer shrink-0" title={isActive ? 'Desactivar' : 'Activar'}>
                                <input type="checkbox" className="sr-only peer" checked={isActive} onChange={() => handleToggleProductStatus(p)} />
                        ...
                        <div className='flex gap-2 pt-3 border-t border-gray-100'>
                            <button onClick={() => { setEditingProduct(p); setShowProductForm(true); }} className='flex-1 flex items-center justify-center gap-1 py-2 text-sm font-medium text-gray-600 bg-gray-50 rounded hover:bg-gray-100 transition-colors'><HiPencil /> Editar</button>
                            <button onClick={() => deleteProduct(p.productoId)} className='px-3 text-red-500 bg-red-50 rounded hover:bg-red-100 transition-colors'><HiTrash /></button>
                        </div>
                    </div>
                  </div>
                );
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 text-gray-600 uppercase font-semibold border-b">
                        <tr><th className="px-6 py-4">ID</th><th className="px-6 py-4">Cliente</th><th className="px-6 py-4">Total</th><th className="px-6 py-4">Estado</th><th className="px-6 py-4 text-center">Acciones</th></tr>
                    </thead>
                        ...
                        {paginatedOrders.map(order => (
                            <tr key={order.pedidoId} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <select value={order.estadoId} onChange={(e) => handleOrderStatusChange(order.pedidoId, e.target.value)}
                                        className={`block w-full py-1 pl-2 pr-6 text-xs font-semibold rounded-full border-0 ring-1 ring-inset focus:ring-2 focus:ring-cyan-600 sm:text-sm sm:leading-6 cursor-pointer ${
                                            ...
                                            order.estadoId === 2 ? 'bg-blue-50 text-blue-700 ring-blue-600/20' :
                                            order.estadoId === 5 ? 'bg-red-50 text-red-700 ring-red-600/20' :
                                            'bg-green-50 text-green-700 ring-green-600/20'}`}>
                                        <option value={1}>Pendiente</option><option value={2}>Procesando</option><option value={3}>Enviado</option><option value={4}>Entregado</option><option value={5}>Cancelado</option>
                                    </select>
                                </td>
      ...
      {activeSection === 'users' && (
        <div className='grid grid-cols-1 xl:grid-cols-3 gap-8 animate-fade-in'>
          <div className='xl:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden'>
             <div className="p-6 border-b border-gray-100 flex justify-between items-center gap-4"><h2 className='font-bold text-gray-800 text-lg'>Usuarios</h2><div className="relative w-full sm:w-64"><HiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input className='w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 outline-none' placeholder='Buscar...' value={userSearch} onChange={e => {setUserSearch(e.target.value); setUserPage(1)}} /></div></div>
